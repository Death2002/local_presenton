...
const setupUserConfigFromEnv = () => {
  ...
  const normalizeImageConfig = (config) => {
    ...
    if (config.IMAGE_PROVIDER) {
      config.DISABLE_IMAGE_GENERATION = false;
      return config;
    }
  ...
  const userConfig = {
    ...
    OPENAI_COMPAT_IMAGE_BASE_URL: process.env.OPENAI_COMPAT_IMAGE_BASE_URL || existingConfig.OPENAI_COMPAT_IMAGE_BASE_URL,
    OPENAI_COMPAT_IMAGE_API_KEY: process.env.OPENAI_COMPAT_IMAGE_API_KEY || existingConfig.OPENAI_COMPAT_IMAGE_API_KEY,
    OPENAI_COMPAT_IMAGE_MODEL: process.env.OPENAI_COMPAT_IMAGE_MODEL || existingConfig.OPENAI_COMPAT_IMAGE_MODEL,
  ...
  const computedConfig = normalizeImageConfig(userConfig);
  writeFileSync(userConfigPath, JSON.stringify(computedConfig));

  process.env.IMAGE_PROVIDER = computedConfig.IMAGE_PROVIDER || "";
  process.env.DISABLE_IMAGE_GENERATION = computedConfig.DISABLE_IMAGE_GENERATION === true ? "true" : "false";

  console.log(
    `[config] IMAGE_PROVIDER=${process.env.IMAGE_PROVIDER} DISABLE_IMAGE_GENERATION=${process.env.DISABLE_IMAGE_GENERATION} OPENAI_COMPAT vars=${computedConfig.OPENAI_COMPAT_IMAGE_BASE_URL ? "present" : "missing"}`
  );
};
